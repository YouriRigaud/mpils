...
class TunerMemory {
    ...
    public:
        ...
        void exportEvaluationLogCSV(const std::string& filename) const {
            ...
            // Write header
            file << "EvalID,TimeEvaluated,SolverRuntimeSeconds,SolverTerminationStatus,ObjectiveValue,Gap,UpperBound,LowerBound,ConfigID,MipStartID,WorkerID,Iteration,Phase\n";
            // Write records
            for (const auto& record : evaluations_) {
                file << record.evaluation_id << ","
                     ...
                     << solverTerminationStatusToString(record.solver_termination_status) << ","
